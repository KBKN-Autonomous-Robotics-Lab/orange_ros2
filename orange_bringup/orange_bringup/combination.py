...
class CLASMovingBaseCombiner(Node):
    def __init__(self):
        ...
        self.mf = ApproximateTimeSynchronizer(
            [self.sub1, self.sub2], queue_size=10, slop=0.5)
    ...
    def publish_combined_odom(self):
        if self.CLAS_position is not None and self.movingbase_yaw is not None:
            ...
            self.odom_msg.pose.pose.position.z = self.CLAS_position.pose.pose.orientation.w
