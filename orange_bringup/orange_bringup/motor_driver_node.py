...
class MotorDriverNode(Node):
  def __init__(self):
    ...
    self.client = ModbusClient(port = self.get_parameter("port").get_parameter_value().string_value, baudrate = 115200, timeout = 1)
    self.client.connect()
    self.ID = 1
    ...
    self.create_subscription(Twist, self.get_parameter("twist_cmd_vel_topic").get_parameter_value().string_value, self.twist_cmd_callback, 10)
    self.create_subscription(Float32MultiArray, self.get_parameter("cmd_vel_topic").get_parameter_value().string_value, self.vel_cmd_callback, 10)
    self.create_subscription(Float32MultiArray, self.get_parameter("cmd_rpm_topic").get_parameter_value().string_value, self.rpm_cmd_callback, 10)
    self.create_subscription(Float32MultiArray, self.get_parameter("cmd_deg_topic").get_parameter_value().string_value, self.deg_cmd_callback, 10)
    self.create_subscription(Float32MultiArray, self.get_parameter("cmd_dist_topic").get_parameter_value().string_value, self.dist_cmd_callback, 10)
    ...
    # -----Initialize Variable-----
    self.callback_timeout = self.get_parameter("callback_timeout").get_parameter_value().double_value
    self.wheels_base_width = self.get_parameter("wheels_base_width").get_parameter_value().double_value
    self.left_wheel_radius = self.get_parameter("left_wheel_radius").get_parameter_value().double_value
    self.right_wheel_radius = self.get_parameter("right_wheel_radius").get_parameter_value().double_value
    self.computation_left_wheel_radius = self.get_parameter("computation_left_wheel_radius").get_parameter_value().double_value
    self.computation_right_wheel_radius = self.get_parameter("computation_right_wheel_radius").get_parameter_value().double_value
    self.cpr = self.get_parameter("cpr").get_parameter_value().integer_value
    self.deadband_rpm = self.get_parameter("deadband_rpm").get_parameter_value().integer_value
    self.left_rpm_lim = self.get_parameter("max_left_rpm").get_parameter_value().integer_value
    self.right_rpm_lim = self.get_parameter("max_right_rpm").get_parameter_value().integer_value
    self.publish_TF = self.get_parameter("publish_TF").get_parameter_value().bool_value
    self.publish_odom = self.get_parameter("publish_odom").get_parameter_value().bool_value
    self.TF_header_frame = self.get_parameter("TF_header_frame").get_parameter_value().string_value
  ...
  def speed_mode_init(self):
    ...
    # -----Set Accel Time-----
    AL_ms = self.get_parameter("set_accel_time_left").get_parameter_value().integer_value
    AR_ms = self.get_parameter("set_accel_time_right").get_parameter_value().integer_value
    if AL_ms > 32767:
      AL_ms = 32767
    elif AL_ms < 0:
      AL_ms = 0
    ...
    result = self.client.write_registers(self.L_ACL_TIME, [AL_ms, AR_ms], slave=self.ID)

    # -----Set Decel Time-----
    DL_ms = self.get_parameter("set_decel_time_left").get_parameter_value().integer_value
    DR_ms = self.get_parameter("set_decel_time_right").get_parameter_value().integer_value
    if DL_ms > 32767:
      DL_ms = 32767
    elif DL_ms < 0:
      DL_ms = 0
    ...
    result = self.client.write_registers(self.L_DCL_TIME, [DL_ms, DR_ms], slave=self.ID)
  ...
  def position_mode_init(self):
    ...
    # -----Set Position Async Control-----
    max_L_rpm = self.get_parameter("max_left_rpm").get_parameter_value().integer_value
    max_R_rpm = self.get_parameter("max_right_rpm").get_parameter_value().integer_value
    if max_L_rpm > 1000:
      max_L_rpm = 1000
    elif max_L_rpm < 1:
      max_L_rpm = 1
    ...
    result = self.client.write_registers(self.L_MAX_RPM_POS, [max_L_rpm, max_R_rpm], slave=self.ID)
  ...
  def control_loop(self):
    ...
    if self.estop:
      ...
      if self.control_mode == 3:
        ...
        result = self.client.write_register(self.CONTROL_REG, self.EMER_STOP, slave=self.ID)
    ...
    #-----Debugging Feature-----
    if (self.debug):
      self.get_logger().info("x: %f | y: %f | yaw: %f" % (x, y, np.rad2deg(theta)))
